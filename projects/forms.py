...
class ProjectForm(forms.ModelForm):
    class Meta:
        ...
        widgets = {
            ...
            "specialist": Select2MultipleWidget(
                attrs={
                    "class": "form-select",
                    "data-placeholder": "Selecciona especialistas",
                }
            ),
    ...
    def __init__(self, *args, **kwargs):
        ...
        if not self.initial.get("code_name") and not self.data.get("code_name"):
            self.fields["code_name"].widget.attrs["placeholder"] = generate_code_name(
                capitalize=True
            )
        ...
        self.fields["specialist"].widget.attrs.update(
            {"data-placeholder": "Asignar especialista"}
        )
        ...
        allowed_users = User.objects.filter(
            role__in=[admin_role.id, specialist_role.id]
        )
